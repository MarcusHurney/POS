...
export default function(state = INITIAL_STATE, action) {
  switch (action.type) {
    case CALCULATE_CART_TOTALS:
      ...
      state.currentCart.forEach(
        ({ price, cartQuantity }) => (cartSubtotal += price * cartQuantity)
      );

      taxTotal += cartSubtotal * state.tax / 100;
      cartTotal += cartSubtotal + taxTotal;

      return { ...state, taxTotal, cartSubtotal, cartTotal };
    ...
    case DECREMENT_PRODUCT_IN_CART:
      ...
      if (productToDelete.cartQuantity > 1) {
        ...
        return {
          ...state,
          currentCart: _.without(state.currentCart, action.payload)
        };
      }
